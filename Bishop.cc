...
Bishop::Bishop(const Grid& grid, const Colour colour, const pair<int, int>& position)
        : Piece(BISHOP, grid, colour, position) {}


shared_ptr<Piece> Bishop::clone(const Grid& grid_clone) const {
    return make_shared<Bishop>(grid_clone, colour, position);
}


vector<Move> Bishop::getVisibleMoves() const {
    vector<Move> moves;
